...
import { encryptMessage, decryptMessage, getSecondsUntilNextKey } from "@/lib/crypto";
...
export default function ChatWindow({
  ...
  const [keyTimer, setKeyTimer] = useState(getSecondsUntilNextKey());
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setKeyTimer(getSecondsUntilNextKey());
    }, 1000);
  ...
  return (
    <div className="flex flex-col flex-1 h-full bg-[#13141c] relative overflow-hidden">
      ...
      {/* Encryption banner */}
      <div className="flex items-center justify-between px-5 py-2 bg-violet-900/20 border-b border-violet-500/10 flex-shrink-0">
        <div className="flex items-center gap-2">
          <Shield className="w-3.5 h-3.5 text-violet-400" />
          <span className="text-[11px] text-violet-300">
            Сообщения зашифрованы (ключ меняется каждую минуту)
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 scrollbar-thin">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 text-white/20">
            ...
            <p className="text-sm">Начните зашифрованный разговор</p>
          </div>
        )}
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            {/* Encrypted preview */}
            {showEncrypted && msg.type === "text" && (
              <div className="max-w-full px-3 py-1.5 rounded-lg bg-black/40 border border-violet-500/20">
                <p className="text-[10px] text-violet-400/60 font-mono break-all">
                  🔒 {msg.encryptedText.slice(0, 40)}…
                </p>
              </div>
            )}
